...
plot([3600 4000 4000 3600 3600],[2000 2000 2400 2400 2000])
...
z0=zi(z_num1);
xji=zeros(1,161);yji=zeros(1,161);
...
%尝试加入隧道
mm=sub2ind(size(zi),480-309+1,440);
mm2=zi(mm);
tunnel_length=0;
while 1
    tunnel_length=tunnel_length+10;
    z_tunnelm=sub2ind(size(zi),480-(tunnel_length/10+309)+1,440);
    z_tunnel=zi(z_tunnelm);
    if(mm2+0.1*tunnel_length>=z_tunnel)
        break;
    end
end
x0=4400;y0=3090+tunnel_length;
%plot([4400 4400],[3090 y0],'linewidth',1.5)
z_num1=sub2ind(size(zi),480-y0/10+1,x0/10+1);
z0=zi(z_num1);
...
fprintf("隧道长度%d\n",tunnel_length)
fprintf("隧道口到矿区价格为%d\n",price)
price3=price2+1500*tunnel_length+price;
fprintf("居民点到矿区价格为%d\n",price3)
%4次实验下不考虑隧道
...
while 1
    y_test=(fix(y0/10)+1)*10;
    x_test=((3600-x0)/(2400-y0))*y_test+x0-y0*((3600-x0)/(2400-y0));
    z_testnum1=sub2ind(size(zi),480-y_test/10+1,fix(x_test/10)+1);
    z_test1=zi(z_testnum1);
    z_testnum2=sub2ind(size(zi),480-y_test/10+1,fix(x_test/10)+2);
    z_test2=zi(z_testnum2);
